...
echo "Migrating to use mechanicus-launch-webapp and mechanicus-launch-browser"
for desktop_file in ~/.local/share/applications/*.desktop; do
  if grep -q 'Exec=chromium --new-window --ozone-platform=wayland --app=' "$desktop_file"; then
    ...
    if [[ -n "$url" ]]; then
      sed -i "s|^Exec=.*|Exec=mechanicus-launch-webapp \"$url\"|" "$desktop_file"
    fi
  fi
done
...
if [ -f "$HYPR_BINDINGS_FILE" ]; then
  sed -i 's/\$browser =.*chromium.*$/\$browser = mechanicus-launch-browser/' "$HYPR_BINDINGS_FILE"
  sed -i 's/\$webapp="/mechanicus-launch-webapp "/g' "$HYPR_BINDINGS_FILE"
