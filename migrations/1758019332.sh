...
mechanicus-refresh-config uwsm/default
mechanicus-refresh-config uwsm/env
mechanicus-state set reboot-required
...
if grep -q "bindd = SUPER, N, Neovim" ~/.config/hypr/bindings.conf; then
  sed -i '/SUPER, N, Neovim, exec/ c\bindd = SUPER, N, Editor, exec, mechanicus-launch-editor' ~/.config/hypr/bindings.conf
fi
