echo -e "Offer new Mechanicus hotkeys\n"
...
if gum confirm "Switch to new hotkeys?"; then
  ...
  sed -i 's|source = ~/.local/share/mechanicus/default/hypr/bindings/tiling\.conf|source = ~/.local/share/mechanicus/default/hypr/bindings/clipboard.conf\
source = ~/.local/share/mechanicus/default/hypr/bindings/tiling-v2.conf|' ~/.config/hypr/hyprland.conf
fi
