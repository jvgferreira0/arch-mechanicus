...
# Users can still opt-in to Cloudflare DNS using: mechanicus-setup-dns cloudflare
...
if [ -f /etc/systemd/resolved.conf ]; then
  ...
  if [ -f /etc/systemd/network/99-mechanicus-dns.network ]; then
    sudo rm -f /etc/systemd/network/99-mechanicus-dns.network
  ...
  echo "To use Cloudflare DNS, run: mechanicus-setup-dns Cloudflare"
fi